...
# Use RTSP stream URL for capturing video feed
rtsp_url = "rtsp://admin:admin123@192.168.1.204:554/avstream/channel=1/stream=1.sdp"  # Replace with your RTSP stream URL
cap = cv2.VideoCapture(rtsp_url)
cap.set(cv2.CAP_PROP_BUFFERSIZE, 3)  # Increase buffer size
cap.set(cv2.CAP_PROP_FPS, 30)  # Set frame rate to 30 (or adjust accordingly)
...
known_faces = {
    "JohnnyBaby": "cutie.jpg",
    "GauriSharmaaaa": "uglyface.jpg"
}
...
# Loop to capture and process video frames from RTSP stream
while True:
    ret, frame = cap.read()
    if not ret:
    ...
    try:
        ...
        results = DeepFace.analyze(frame_rgb, actions=['emotion', 'race'], detector_backend='opencv', enforce_detection=False)
    ...
    # Show Video Feed from RTSP Stream
    resize = cv2.resize(frame,(640,640))
    cv2.imshow("Face Recognition (RTSP Stream + FAISS)", resize)
